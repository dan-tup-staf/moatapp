...
## 🔑 Klucze API do dostarczenia (gdy będziesz gotów)

12. **Wzbogacanie danych (e-maile/telefony)** — chcesz integrację z **Apollo**,
    **Lusha** i **Prospeo**. Szkielet zbuduję, ale do działania potrzebne klucze:
    - **Apollo.io** API key (Settings → Integrations → API).
    - **Lusha** API key (plan z dostępem do API).
    - **Prospeo** API key (prospeo.io → API).
    Podasz je w ustawieniach aplikacji (będą szyfrowane). Bez nich integracje są
    nieaktywne (placeholder „podłącz dostawcę”).
13. **Stripe (płatności)** — do landingu + freemium + planów płatnych:
    - `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, oraz `price_...` ID planów
      (Pro, Scale) z dashboardu Stripe.
    - Bez nich billing działa w trybie podglądu (plany widoczne, brak realnej
      płatności).
